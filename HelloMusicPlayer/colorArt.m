...
uint8_t gs_pixel_avg (gs_pixcel* p){
    int16_t a = (p->r+p->g+p->b)/3;
    return (uint8_t)(a>0xFF)?0xFF:a;
}

UIColor* dataColorAtXY(const gs_pixcel* imgData,int pixelsPerRow,NSUInteger x, NSUInteger y){
    gs_pixcel pixel = imgData[y*pixelsPerRow + x];
...
-(id)initWithImage:(UIImage *)image scaledSize:(CGSize)size{
    ...
    if (self) {
        if (CGSizeEqualToSize(size, CGSizeZero)||CGSizeEqualToSize(size, image.size)) {
            ...
            UIImage* finalImage = [self scaledImage:image size:size];
...
- (UIImage*)scaledImage:(UIImage*)image size:(CGSize)targetSize
{
    ...
    UIGraphicsBeginImageContext(targetSize); //画面外に描画領域を作成
    ...
    newImage = UIGraphicsGetImageFromCurrentImageContext(); //画面外領域をUIImageに
    
    if (newImage == nil) {
        NSLog(@"could not scale image");
    }
    
    //pop tha context to get back to the defaut
    UIGraphicsEndImageContext();  //画面外領域を削除
...
- (void)analyzeImage:(UIImage*)anImage
{
    ...
    [self findTextColors:imageColors primaryColor:&primaryColor secondaryColor:&secondaryColor datailColor:&detailColor backgroundColor:backgroundColor];
    ...
    *colors = imageColors;
    ...
    while ((curColor = [enumerator nextObject])!= nil) {
        ...
        if (colorCount <= randomColorsThreshold) {
           // threshold based on input image height
            continue;
        }
    ...
    PCCountedColor* proposeEdgeColor = nil;
    
    if ([sortedColors count]>0) {
        proposeEdgeColor = [sortedColors objectAtIndex:0];
        if ([proposeEdgeColor.color pc_isBlackOrWhite]) {
            for (NSInteger i=1; i<[sortedColors count]; i++) {
                PCCountedColor* nextProposedColor = [sortedColors objectAtIndex:i];
                if (((double)nextProposedColor.count/(double)proposeEdgeColor.count)>.3)
                    //make sure the second choice color is 30% as common as the first choice
                {
                    if (![nextProposedColor.color pc_isBlackOrWhite]) {
                        proposeEdgeColor = nextProposedColor;
                        break;
                    }
                }
                else{
    ...
    return proposeEdgeColor.color;
    
    
}
- (void)findTextColors:(NSCountedSet*)colors primaryColor:(UIColor**)primaryColor secondaryColor:(UIColor**)secondaryColor datailColor:(UIColor**)detailColor backgroundColor:(UIColor*)backgroundColor
{
    ...
    [sortedColors sortUsingSelector:@selector(compare:)];
    for (PCCountedColor* curContainar in sortedColors) {
        curColor = curContainar.color;
